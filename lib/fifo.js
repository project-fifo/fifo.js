...
function post(path, body, token) {
    ...
    var c = client(token.endpoint);
    console.log(fifoPath[token.endpoint] + path);
    c.post({
        path: fifoPath[token.endpoint] + path,
        headers: {
            'x-snarl-token': token.token
...
function check_endpoint(endpoint, callback) {
    try {
        ...
        function testPort(options, callback) {
            ...
            var timeout_wrapper = function (req) {
                    return function() {
                        ...
                        console.log('Endpoint timeout, trying next endpoint');
                        req.abort();
                        callback(false);
                    };
                },
                request = http.get(options.endpoint, function (res) {
                    ...
                    res.on("error", function(err) {
                        ...
                        console.log('Connectivity not found:', err);
                        setTimeout(function() {
                            callback(false);
                        }, 1);
                    });
                    res.on("data", function(chunk) {
                        clearTimeout(timeout);
                        console.log('Connectivity confirmed!');//, chunk);
                        callback(true); 
                    });
                    res.on("end", function() {
                        clearTimeout(timeout);
        ...
        console.log('check_endpoint error:', err);
    }
}
