...
function post(path, body, token) {
    ...
    deferred.promise.then(function(location) {
        if ((location instanceof Array) && (location.length === 0)) {
            deferred2.resolve(location);
        }
        c.get({
            path: location,
            headers: {
    ...
    try {
        ...
        var secure = (matches[1] === "https://"),
            ...
            options = {
                host: host,
                port: port,
                path: path + defaultFifoPath + '/cloud/connection'
            }
...
FiFo = {
    connect: function(endpoint, user, password, saveDisabled) {
        ...
        checkEndpoint(endpoint, function(worked) {
            if (!worked) {
                deferred.resolve({
                    user: -1,
                    endpoint: endpoint
                ...
                var item1 = function() {
                        ...
                        console.log('Logging into:', fifoPath[endpoint] + "/sessions");
                        ...
                        else
                        {
                            ...
                            c.get({
                                ...
                                if (err) {
                                    ...
                                    deferredB.resolve(savedToken[user]);
                                }
                            });
                        }
    ...
    signup: function(endpoint, dataStruct) {
        var deferred = Q.defer();
        var c = client(endpoint);
        console.log('Signing up:', fifoPath[endpoint] + "/dozer", dataStruct);
        checkEndpoint(endpoint, function(worked) {
            if (worked) {
                c.post(fifoPath[endpoint] + "/dozer", dataStruct, function(err, req, res, obj) {
                    if (err) {
                        console.log('Error family:', (err.statusCode / 100).toFixed(0));
                        if (((err.statusCode / 100).toFixed(0) == 4) || ((err.statusCode / 100).toFixed(0) == 5))
                        {
                        ...
                        deferred.resolve({success: 'success', location: res.headers.location, status: res.body});
                    }
                });
            } else {
                deferred.resolve({
...
User = {
    ...
    listGroups: function(id, token) {
        return get("/users/" + id + "/groups", token);
    },
    joinGroup: function(id, group, token) {
        return put("/users/" + id + "/groups/" + group, {}, token);
    },
    leaveGroup: function(id, group, token) {
...
Group = {
    ...
    metadataSet: function(id, key, value, token) {
        return metadataSet("groups", id, key, value, token);
    },
    metadataDel: function(id, path, token) {
        return metadataDel("groups", id, path, token);
    }
};
...
Organization = {
    ...
    addTrigger: function(id, triggerGroup, triggerPayload, token) {
        return put("/orgs/" + id + "/triggers/" + triggerGroup, triggerPayload, token);
    },
    removeTrigger: function(id, triggerGroup, triggerPayload, token) {
        console.error("Not working properly yet");
        // throw Error("Not supported");
        return del("/orgs/" + id + "/triggers/" + triggerGroup, triggerPayload, token);
    },
    metadataSet: function(id, key, value, token) {
